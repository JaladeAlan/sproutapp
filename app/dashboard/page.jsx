...
import { TrendingUp, Wallet, MapPin, Activity, ArrowUpRight, LayoutGrid } from "lucide-react";
...
const amountMeta = (type = "") => {
  ...
  if (t.includes("withdraw") || t.includes("purchase") || t.includes("invest")) return { sign: "−", color: "text-red-400" };
...
export default function Dashboard() {
  ...
  // Show welcome toast ONLY on the first visit after login.
  useEffect(() => {
    if (!user) return;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (err.response?.status === 401) { localStorage.removeItem("token"); router.replace("/login"); }
  ...
  return (
    <div className="min-h-screen bg-[#0D1F1A] relative"
      ...
      {/* Dot grid */}
      <div className="fixed inset-0 pointer-events-none" style={{ backgroundImage: "radial-gradient(circle, #fff 1px, transparent 1px)", backgroundSize: "28px 28px", opacity: 0.03 }} />
      {/* Glows */}
      <div className="fixed top-[-20%] right-[-10%] w-[50vw] h-[50vw] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, #C8873A 0%, transparent 70%)", opacity: 0.1 }} />
      <div className="fixed bottom-[-20%] left-[-10%] w-[40vw] h-[40vw] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, #2D7A55 0%, transparent 70%)", opacity: 0.08 }} />
      ...
      <main className="relative max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {loadingStats ? [1,2,3,4].map(i => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/5 h-32 animate-pulse" />
          )) : (
            <>
              <StatCard icon={<Wallet size={18} />}     label="Total Balance"   value={formatCurrency(stats?.balance)}         full={`₦${Number(stats?.balance ?? 0).toLocaleString()}`}         accent="amber"   />
              <StatCard icon={<TrendingUp size={18} />}  label="Invested"        value={formatCurrency(stats?.total_invested)}  full={`₦${Number(stats?.total_invested ?? 0).toLocaleString()}`}  accent="emerald" />
              <StatCard icon={<MapPin size={18} />}      label="Lands Owned"     value={stats?.lands_owned ?? 0}                sub={`${stats?.units_owned ?? 0} total units`}                     accent="blue"    />
              <StatCard icon={<Activity size={18} />}    label="Total Withdrawn" value={formatCurrency(stats?.total_withdrawn)} sub={`${stats?.pending_withdrawals ?? 0} pending`}                accent="purple"  />
            </>
          )}
        </div>
...
function StatCard({ icon, label, value, full, sub, accent }) {
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-5 transition-colors" style={{ cursor: "default" }}
      onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.08)"}
      onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}>
      <div className={`w-9 h-9 rounded-xl ${a.bg} border ${a.border} flex items-center justify-center mb-4 ${a.icon}`}>{icon}</div>
      <p className="text-xs font-medium uppercase tracking-wider mb-1" style={{ color: "rgba(255,255,255,0.3)" }}>{label}</p>
...
function QuickCard({ title, desc, href, cta, icon }) {
  return (
    <Link href={href} className="rounded-2xl border border-white/10 bg-white/5 p-5 transition-all group block"
      onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.08)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.15)"; }}
      onMouseLeave={e => { e.currentTarget.style.background = "rgba(255,255,255,0.05)"; e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)";  }}>
      <div className="w-9 h-9 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-500 mb-4">{icon}</div>
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
        ...
        <div className="space-y-3">
          {[...Array(4)].map((_, i) => <div key={i} className="h-14 rounded-xl animate-pulse" style={{ background: "rgba(255,255,255,0.05)" }} />)}
        </div>
      </div>
    );
  }
  ...
  if (!transactions?.length) {
    return (
      <div className="rounded-2xl border border-white/10 bg-white/5 p-12 text-center">
        <div className="w-14 h-14 rounded-2xl border border-white/10 flex items-center justify-center mx-auto mb-4" style={{ background: "rgba(255,255,255,0.05)" }}>
          <Activity size={22} className="text-white/20" />
        </div>
        <h3 className="font-bold text-white mb-1.5">No transactions yet</h3>
        <p className="text-sm mb-6" style={{ color: "rgba(255,255,255,0.3)" }}>Start investing in land to see your transaction history</p>
        <Link href="/lands" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm text-[#0D1F1A]"
          style={{ background: "linear-gradient(135deg, #C8873A 0%, #E8A850 100%)" }}>
          Browse Lands <ArrowUpRight size={14} />
        </Link>
      </div>
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-4 border-b border-white/5" style={{ background: "rgba(255,255,255,0.02)" }}>
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.5)" }}>Recent Transactions</h2>
        </div>
        <Link href="/wallet" className="text-xs text-amber-500 hover:text-amber-400 font-semibold transition-colors">View all →</Link>
      </div>
          ...
          <thead>
            <tr className="border-b border-white/5">
              {["Type", "Asset", "Amount", "Status", "Date"].map(h => (
                <th key={h} className={`px-5 py-3 text-xs font-bold uppercase tracking-widest ${h === "Amount" ? "text-right" : "text-left"}`} style={{ color: "rgba(255,255,255,0.25)" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {transactions.slice(0, 6).map((tx, idx) => {
              ...
              return (
                <tr key={tx?.id ?? idx} className="border-b border-white/5 transition-colors"
                  onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.03)"}
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <td className="px-5 py-4 font-medium capitalize" style={{ color: "rgba(255,255,255,0.7)" }}>{tx?.type?.replace("_", " ")}</td>
                  <td className="px-5 py-4" style={{ color: "rgba(255,255,255,0.4)" }}>{tx?.land || "Wallet"}</td>
                  <td className={`px-5 py-4 text-right font-bold tabular-nums ${color}`}>{sign}₦{Number(tx?.amount ?? 0).toLocaleString()}</td>
                  <td className="px-5 py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border capitalize ${cls}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />{tx?.status || "-"}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-xs" style={{ color: "rgba(255,255,255,0.25)" }}>{formatDate(tx?.date)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="md:hidden" style={{ borderTop: "none" }}>
        {transactions.slice(0, 5).map((tx, idx) => {
          ...
          const { cls, dot }    = statusCfg(tx?.status);
          return (
            <div key={tx?.id ?? idx} className="px-4 py-4 transition-colors border-b border-white/5"
              onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.03)"}
              onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="font-semibold text-sm capitalize" style={{ color: "rgba(255,255,255,0.7)" }}>{tx?.type?.replace("_", " ")}</p>
                  <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.3)" }}>{tx?.land || "Wallet"}</p>
                </div>
                <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-bold border capitalize ${cls}`}>
                  <span className={`w-1 h-1 rounded-full ${dot}`} />{tx?.status}
                </span>
              </div>
              <div className="flex items-center justify-between mt-1">
                <p className={`text-lg font-bold tabular-nums ${color}`}>{sign}₦{Number(tx?.amount ?? 0).toLocaleString()}</p>
                <p className="text-xs" style={{ color: "rgba(255,255,255,0.2)" }}>{formatDate(tx?.date)}</p>
              </div>
            </div>
          );
        })}
