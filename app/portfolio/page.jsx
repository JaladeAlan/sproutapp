...
export default function Portfolio() {
  ...
  const closeModal = () => setModal({ type: null, land: null, units: "", pin: "", processing: false });

  const handleTransaction = async (e) => {
    e.preventDefault();
    const units = Number(modal.units);
    if (!units || units <= 0) return toast.error("Invalid units");
    if (modal.type === "sell" && units > modal.land.units_owned) return toast.error("Cannot sell more than owned");
    if (modal.pin.length !== 4) return toast.error("Enter 4-digit PIN");
